...
function EncodingParametersInURLs() {
    ...
    const fetchSum = async (a: number, b: number) => {
        console.log("In fetch sum")
        try {
            const response = await axios.get(`${API_BASE}/a5/add/${a}/${b}`);
            console.log("Response from fetchSum:", response.data);
    ...
    const fetchSubtraction = async (a: number, b: number) => {
        try {
            ...
            console.log("Response from fetchSubtraction:", response.data);
            setResult(response.data);
        } catch (error) {
    ...
    return (
        <div>
            ...
            <h3>Fetch Result</h3>
            <button onClick={() => fetchSum(a, b)} className="btn btn-primary me-2">
                Fetch Sum of {a} + {b}
            </button>
            <button onClick={() => fetchSubtraction(a, b)} className="btn btn-danger me-2">
                Fetch Substraction of {a} - {b}
            </button>

            <h3>Path Parameters</h3>

            <a href={`${API_BASE}/a5/add/${a}/${b}`} className="btn btn-primary me-2">Add {a} + {b}</a>
            <a href={`${API_BASE}/a5/subtract/${a}/${b}`} className="btn btn-danger me-2">Subtract {a} - {b}</a>
            <a href={`${API_BASE}/a5/multiply/${a}/${b}`} className="btn btn-secondary me-2">Multiply {a} * {b}</a>
            <a href={`${API_BASE}/a5/divide/${a}/${b}`} className="btn btn-success">Divide {a} / {b}</a>
